...
const COPY: Record<PopupLanguage, Copy> = {
  en: {
    ...
    fetchSessionExpired: (label) => `Could not read ${label}: DX NET reports that the connection has expired (200002). Sign in to DX NET again, refresh its home page, and retry. If it persists, try again later.`,
  ...
  "zh-Hant": {
    ...
    fetchSessionExpired: (label) => `無法讀取${label}：DX NET 回報連線已過期（200002）。請重新登入 DX NET、重新整理首頁後再收集。若仍出現，請稍後再試。`,
  ...
  ja: {
    ...
    fetchSessionExpired: (label) => `${label}を読み込めませんでした：DX NET が接続期限切れ（200002）を報告しました。DX NET に再ログインし、ホームを再読み込みしてから再試行してください。続く場合は時間をおいて再試行してください。`,
