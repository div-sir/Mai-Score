...
interface TierEntry {
  min: number;
  tier: RatingTier;
  // Present for tiers that carry a 0-4 star sub-rank (gold and up, per the
  // official table). Stars advance every `step` points above `min`, capped
  // at `max`.
  stars?: { step: number; max: number };
}

// 13000 and up confirmed directly against the official in-game rating table
// (band edges, colours, and the star sub-rank thresholds all read from it).
// Below 13000 is still community-documented guesswork, not re-verified —
// flag any of those if they look wrong once rendered.
...
// 0 for every tier without a star sub-rank (below gold). Otherwise 1-{max},
// advancing every `step` points into the tier.
