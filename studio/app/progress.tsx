    ...
    <section className="progress-dashboard">
      ...
      <article className="rating-overview-card">
        <div className="rating-overview-primary">
          <div className="metric-label"><span>B50</span><small>{copy.last30Days} {signed(periodDelta(timeline, "b50", 30))}</small></div>
          <strong>{latestPoint.b50}</strong>
          <small>{copy.allTime} <b className={(periodDelta(timeline, "b50") ?? 0) >= 0 ? "up" : "down"}>{signed(periodDelta(timeline, "b50"))}</b></small>
        </div>
        <dl>
          <div><dt>New B15</dt><dd>{latestPoint.b15}</dd><small>{copy.last30Days} {signed(periodDelta(timeline, "b15", 30))}</small></div>
          <div><dt>Old B35</dt><dd>{latestPoint.b35}</dd><small>{copy.last30Days} {signed(periodDelta(timeline, "b35", 30))}</small></div>
        </dl>
      </article>
      ...
      <div className="insight-grid">
        ...
        {actionView === "protect" ? <article className="insight-panel action-panel cutoff-panel">
